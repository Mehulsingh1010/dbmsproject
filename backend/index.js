...
const dbConfig = {
  user: "system",
  password: "q",
  connectString: "LAPTOP-ORK2PTHC:1521/XE",
};
  ...
  try {
    ...
    for (const subject of subjects) {
      ...
      if (isNaN(max_marks) || isNaN(scored_marks)) {
        throw new Error(
          `Invalid marks provided: max_marks=${max_marks}, scored_marks=${scored_marks}`
        );
      }
      ...
      if (parseFloat(scored_marks) > parseFloat(max_marks)) {
        throw new Error(
          "Check details once again: Scored marks cannot be greater than max marks."
        );
      }
